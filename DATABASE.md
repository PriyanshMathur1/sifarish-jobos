...
- **Job graph**: companies (ATS identity, consecutive_failures for persistent circuit-break), jobs (source facts vs Sifarish observations kept separate; `market_eligibility`; generated tsvector + trigram index), job_sources, job_versions (full snapshot per content change), refresh_runs (companies_total vs companies_processed drives completion), crawl_errors, user_job_events (append-only behaviour log).
